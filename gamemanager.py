...
def play_turn(board, player_name, opponent_ships):
    ...
    try:
        ...
        if hit:
            ...
            print(f"DEBUG: Hit symbol: '{ship_symbol}'")
            board[ord(x[0]) - 65][int(x[1:])] = " X"  
            for s in opponent_ships:
                print(f"DEBUG: Checking ship {s['name']} (symbol: '{s['symbol']}')")
                if s['symbol'] == ship_symbol: 
                    print(f"DEBUG: Incrementing hits for {s['name']}")
                    s['hits'] += 1
                    if s['hits'] == s['length']:
                        print(f"{s['name']} has been sunk!")
                        print(f"{player_name} wins!")
                        return True  # Game over
                    break  # Stop after incrementing the matching ship
        else:
            print("Missed!")
